...
uninstall() {
    ...
    echo ""
    echo -e " Uninstall successful, if you want to delete this script, run ${green} rm /usr/bin/x-ui -f ${plain} to delete after exiting the script "
    echo ""
...
ssl_cert_issue() {
    ...
    echo -E ""
    LOGD " ******Instructions****** "
    LOGI " This script provides two ways to implement certificate issuance, and the certificate installation path is /root/cert "
    LOGI " Mode 1: acme standalone mode, need to keep the port open "
    LOGI " Mode 2: acme DNS API mode, need to provide Cloudflare Global API Key "
    ...
    read -p " Please select the method you want to use, enter the number 1 or 2 and press Enter " : method
...
install_acme() {
    cd ~
    LOGI " Start installing acme script... "
...
ssl_cert_issue_standalone() {
    ...
    LOGD " The domain name you entered is: ${domain} , the validity of the domain name is being checked... "
    ...
    if [[ ${WebPort} -gt 65535 || ${WebPort} -lt 1 ]]; then
        LOGE " The port ${WebPort} you selected is invalid, the default port 80 will be used for application "
    fi
    LOGI " Will use ${WebPort} for certificate request, please make sure the port is open... "
    #NOTE:This should be handled by user
    #open the port and kill the occupied progress
    ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
    ~/.acme.sh/acme.sh --issue -d ${domain} --standalone --httpport ${WebPort}
    if [ $? -ne 0 ]; then
        LOGE " Certificate application failed, please refer to the error message "
        exit 1
    else
...
ssl_cert_issue_by_cloudflare() {
    ...
    if [ $? -eq 0 ]; then
        ...
        LOGD " Your domain name is set to: ${CF_Domain} , the domain name validity is being checked... "
        #here we need to judge whether there exists cert already
        local currentCert=$(~/.acme.sh/acme.sh --list | tail -1 | awk '{print $1}')
...
————————————————
  ...
  ${green} 7. ${plain} View current panel information
————————————————
